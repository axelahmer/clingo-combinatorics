...
def print_matrix(costs, assignments=None):
    n = costs.shape[0]
    ...
    for i in range(n):
        ...
        for j in range(n):
            cost = costs[i, j]
            if assignments and (i+1, j+1) in assignments:
                print(colored(f"{cost:>{col_width}d}",
...
def print_solution(model, costs, solve_time):
    assignments = [(atom.arguments[0].number, atom.arguments[1].number)
...
def main(n):
    ...
    ctl.load("base.lp")
    ctl.add("base", [], "\n".join(f"cost({i},{j},{cost})." for i, row in enumerate(
        costs, 1) for j, cost in enumerate(row, 1)))
    ctl.ground([("base", [])])
    ...
    with ctl.solve(yield_=True) as handle:
        ...
        print(colored(f"Total time: {
              time.time() - start_time:.2f}s", "magenta"))


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Solve the Assignment Problem")
    parser.add_argument("-n", type=int, default=5,
                        help="Number of agents and tasks (default: 5)")
